We first create the gtf file with the highest expressed two isoforms. We next filtered out those with different CDS start. The reason to do this is because the main ORFs from the early start isoforms could be wrongly identified as uORFs.  
